 ...
 */
//由数组支持的（环形）有界阻塞队列 FIFO
//试图将一个元素放入一个满队列将导致操作阻塞；从空队列中取出一个元素的尝试也会类似地阻塞。
//并发控制使用经典的双条件算法
public class ArrayBlockingQueue<E> extends AbstractQueue<E>
    ...
    /** The queued items */
    //底层存储元素的数组 final修饰 初始化后 不能再改变其引用指向其他实例(也即容量无法更改)
    final Object[] items;

    //循环队列需要的两个指针
    /** items index for next take, poll, peek or remove */
    //take poll peek remove时的索引
    int takeIndex;
    ...
    private E dequeue() {
        ...
        //提取完数据后，说明数组中有空位，所以可以唤醒 notFull 条件对象的等待队列(链表)中的一个可用线程去 put 数据
        notFull.signal();
        return x;
    }
     ...
     */
    //add方法添加元素
    //调用父类 抽象类AbstractQueue的add方法
    //队列满会抛异常 IllegalStateException
